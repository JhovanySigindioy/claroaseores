...
const PricingCards = () => {
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (scrollRef.current) {
      ...
      const scrollAmount = 300; // Aproximadamente el ancho de una card
    ...
    <section id="planes" className="py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-500 max-w-2xl mx-auto">Explora nuestros planes de Fibra Óptica y Entretenimiento.</p>
        </div>

        {/* Portfolio / Carousel Container */}
        <div className="relative group px-2 sm:px-0">

          {/* BOTÓN IZQUIERDA - Centrado verticalmente */}
          <button
            onClick={() => scroll('left')}
            className="absolute left-0 lg:-left-6 top-1/2 -translate-y-1/2 z-40 p-3 rounded-full bg-white shadow-2xl text-slate-600 hover:bg-claro-red hover:text-white transition-all flex items-center justify-center border border-slate-100 focus:outline-none"
            aria-label="Anterior"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>

          {/* BOTÓN DERECHA - Centrado verticalmente */}
          <button
            onClick={() => scroll('right')}
            className="absolute right-0 lg:-right-6 top-1/2 -translate-y-1/2 z-40 p-3 rounded-full bg-white shadow-2xl text-slate-600 hover:bg-claro-red hover:text-white transition-all flex items-center justify-center border border-slate-100 focus:outline-none"
          ...
          <div
            ref={scrollRef}
            className="flex overflow-x-auto gap-6 pb-12 snap-x snap-mandatory no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0 scroll-smooth"
            style={{ scrollSnapType: 'x mandatory' }}
          >
            {homePlans.map((plan) => (
              <div
                key={plan.id}
                className={`flex-none w-[280px] sm:w-[320px] snap-center relative p-6 rounded-[28px] border-2 transition-all flex flex-col h-[520px] ${plan.recommended
              ...
              >
                ...
                <ul className="space-y-3 mb-6 flex-grow overflow-hidden">
                  {plan.features.slice(0, 6).map((feature, idx) => (
                    <li key={idx} className="flex items-start text-slate-600 text-xs font-medium">
                      <div className={`w-4 h-4 rounded-full flex items-center justify-center mr-2 mt-0.5 flex-shrink-0 ${plan.recommended ? 'bg-claro-red' : 'bg-green-100'
                        }`}>
                ...
                <button
                  ...
                  className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all ${plan.recommended
        ...
        <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 text-center text-[10px] text-slate-500 italic max-w-3xl mx-auto leading-tight">
